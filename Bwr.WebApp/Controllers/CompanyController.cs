...
namespace Bwr.WebApp.Controllers
{
    public class CompanyController : Controller
    {
        ...
        public ActionResult GetCountriesForCompany(int companyId)
        {
            var countries = _companyCountryAppService.GetCountriesForCompany(companyId);

            return Json(new { data = countries }, JsonRequestBehavior.AllowGet);
        }

        [HttpGet]
        public ActionResult CountryCompany(int companyId)
        {
            var dto = new CompanyCountryDto()
            {
                CompanyId = companyId
            };
            return PartialView("_CountryCompany", dto);
        }
        ...
        [HttpPost]
        public ActionResult AddCountryCompany(CompanyCountryDto dto)
        {
            var exist = false;
